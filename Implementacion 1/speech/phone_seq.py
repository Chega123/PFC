...
def get_token_id(txt_file, post_word, is_eof, phone_dict):
    """ Return the token id (token or silence) that exist before post_word. """
    
    # Si hay una palabra posterior, remover puntuación
    if len(post_word) > 0:
    ...
    if txt_f_pt.exists():
        ...
        if is_eof:
            # Si es el final del archivo, verificar si termina con un token
            new_line = line.replace(" ", "").replace("\n", "")
            
            # Si el último carácter es un corchete de cierre ]
            if new_line[-1] == "]":
                match = re.search(r'\[(\d+)\]', new_line)
                if match:
                    token_id_str = match.group(1)
                    token_id = phone_dict.get(token_id_str.upper(), 0)
                else:
                    token_id = 0
            else:
                token_id = 0
        else:
            # Verificar si la palabra anterior era un token
            words = line.split(" ")
            prev_word = None
            for word in words:
                if post_word == word:
            ...
            if prev_word is None:
                ...
                # Verificar si prev_word tiene corchetes []
                match = re.search(r'\[(\d+)\]', prev_word)
                if match:
                    token_id_str = match.group(1)
