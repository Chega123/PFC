...
def save_spect(file_list, dir_name, subdir, a, b, mel_basis, min_level, target_dir, config):
    """ Saves a spectrogram as a compressed npz file """
    ...
    for file_name in sorted(file_list):
        ...
        # Save spectrogram as compressed .npz file
        np.savez_compressed(os.path.join(target_dir, subdir, file_name[:-4]),
                            spectrogram=S.astype(np.float32))
