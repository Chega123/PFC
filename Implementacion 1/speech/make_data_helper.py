...
def get_emotion_class(speaker_dir, file_name, config):
    ...
    emotion_class = 0
    label_path = os.path.join(config.labels_dir, speaker_dir, file_name[:-4] + ".csv")
    
    with open(label_path) as csvfile:
        reader = csv.reader(csvfile)
        for row in reader:
            label = row[-1]  # Assuming 'neu' is in the last column
            break

    # Map emotion to class
    if label.lower() in ("happy", "excited"):
        emotion_class = 2
...
def get_content_list(file_list, speaker_dir, config):
    ...
    This function is now a generator that yields each content element.
    """
    unsuccess_cases = 0
    success_cases = 0
    ...
    for file_name in sorted(file_list):
        ...
        data = np.load(str(config.root_dir) + '/' + speaker_dir + '/' + file_name[:-4] + '.npz')
        ...
        if pathlib_phone_path.exists():
            ...
            if success:
                ...
                utt_content = [str(utt_file), phones_and_durations, main_phones, emotion_class, word_seq, word_intervals]
                success_cases += 1
                yield utt_content  # Yield instead of appending to a list
            else:
                unsuccess_cases += 1
        else:
            ...
            if txt_f_pt.exists():
                if is_file_unk_token(txt_f):
                    ...
                    utt_file = os.path.join(speaker_dir, file_name)
                    emotion_class = get_emotion_class(speaker_dir, file_name, config)
                    utt_content = [str(utt_file), phones_and_durations, main_phones, emotion_class, [None], [None]]
                    success_cases += 1
                    yield utt_content  # Yield instead of appending to a list
                else:
...
def get_speaker_embeddings(dir_name, speaker, file_list, idx_uttrs, num_uttrs, batch_size=25):
    """ Return speaker embeddings from a speaker encoder, processing in batches """

    utterance_embeds = []

    # Procesar en lotes en lugar de todos los archivos a la vez
    for i in range(0, num_uttrs, batch_size):
        batch_idx = idx_uttrs[i:i + batch_size]  # Obtener el índice del lote actual
        wav_files = []
        
        for idx in batch_idx:
            file_name = file_list[idx][:-4] + ".wav"
            wav_file = str(dir_name) + "/" + str(speaker) + "/" + str(file_name)
            wav_files.append(wav_file)

        # Procesar solo el lote de archivos actual
        speaker_wavs = np.array(list(map(preprocess_wav, tqdm(wav_files, "Preprocessing wavs", len(wav_files)))))
        batch_embeds = list(map(spk_encoder.embed_utterance, speaker_wavs))
        
        # Agregar embeddings procesados
        utterance_embeds.extend(batch_embeds)
    ...
    return np.array(utterance_embeds)
