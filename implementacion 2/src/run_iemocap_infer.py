...
def evaluate_metrics(pred_label, true_label, output_path="confusion_matrix.png"):
    # Convertir las etiquetas a formato NumPy
    pred_label = np.array(pred_label)
    true_label = np.array(true_label)
    
    # Cálculo de Unweighted Accuracy (UA) y Weighted Accuracy (WA)
    ...
    # Crear y guardar la gráfica de la matriz de confusión
    ...
    # Reporte por emoción
    class_report = classification_report(true_label, pred_label, target_names=emotions, output_dict=True)
    
    # Extraer el F1 general
    f1_general = class_report["macro avg"]["f1-score"]
    recall=class_report["macro avg"]["recall"]
    print(f"F1 General del modelo: {f1_general:.4f}")
    print(f"Recall: {recall:.4f}")
    
    # Extraer porcentajes por emoción
    emotion_accuracies = {emotion: report['precision'] for emotion, report in class_report.items() if isinstance(report, dict)}
    
    # Métrica clave y reporte general
    key_metric, report_metric = 0.9 * wa + 0.1 * ua, {
        'wa': wa,
        'ua': ua,
        'conf_matrix': conf_matrix,
        'emotion_accuracies': emotion_accuracies,
        'conf_matrix_path': output_path
    }
...
def collate(sample_list):
    ...
    with audio_processor.as_target_processor():
        label_features = audio_processor.tokenizer(batch_asr_text, return_attention_mask=False, padding=True).input_ids
...
def run_infer(config, train_data, valid_data, checkpoint_path, session):
    ...
    with torch.no_grad():
        ...
        key_metric, report_metric = evaluate_metrics(pred_y, true_y, output_path=f"/media/chega/Nuevo vol/mmer_pfc/MMER/results/confusion_matrix_session_{session}.png")

        # Imprimir la ruta de la matriz de confusión
        print(f"\nMatriz de confusión guardada en: {report_metric['conf_matrix_path']}")
        
        # Imprimir precisión por emoción
        print("\nPrecisión por emoción:")
        for emotion, accuracy in report_metric['emotion_accuracies'].items():
