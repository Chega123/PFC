...
audio_processor = create_processor("facebook/wav2vec2-base")  # Revisa cómo está definida `create_processor`.
...
def run(args, config, train_data, valid_data, session):
    ...
    if torch.cuda.is_available():
        device = torch.device("cuda")  # Selecciona CUDA si está disponible
        model = model.to(device)  # Carga el modelo en el dispositivo
        print(f"Using CUDA device: {torch.cuda.current_device()} - {torch.cuda.get_device_name(torch.cuda.current_device())}")
    else:
        device = torch.device("cpu")  # Si no hay CUDA, usa CPU
